<template lang="html">
  ...
  <div v-if='reso' class="app-w">
    ...
    <!-- <div class="l-title">
       ...
       <p @click='change(1)'>产品详情</p>
    </div>

    <div :style='yin' class="tr1">
       <p class="th1">产品名称</p>
       <p class="th2">{{accidentone}}</p>
    ...
    <div :style='yin'>
      ...
      <div class="suggestion">
          <div class="suggestion-left">
              <div class="">
                  <h2 v-if='rtt.premium.substr(0,rtt.premium.indexOf(".00"))'>{{rtt.premium.substr(0,rtt.premium.indexOf('.00'))}}</h2>
                  <h2 v-else>0</h2>
                  <p>元/年</p>
              </div>
              <p>保费</p>
          ...
          <div class="suggestion-right">
             ...
             <div class="three">
               <div class="one-left">
                 <span>性别:</span>
                 <p>{{genderp[rty.gender+'']}}</p>
               </div>
               <div class="one-left">
                 <span>是否抽烟:</span>
                 <p>{{smoke[rty.isSmoking+'']}}</p>
               </div>
      ...
      <div v-for='(item,index) in data' class="product-zong">
        <div class="product">
           ...
           <!-- <span class="span2" v-show='show2' @click='heshang(item.id)'></span> -->
        </div>
...
mounted:{
  ...
  created(){
    ...
    // localStorage.setItem('showproduct', JSON.stringify(this.$route.query.chanpin))
    // var kl = JSON.parse(localStorage.getItem('zhongproduct'));
    // console.log(kl);
      that.label = k.label;
    that.tyzz = this.$route.query.buty;
    console.log(JSON.parse(localStorage.getItem('chanPinData')));
    that.rtt =  JSON.parse(localStorage.getItem('chanPinData'));
    that.rty = JSON.parse(localStorage.getItem('ageXIng'));
  var plansId = JSON.parse(localStorage.getItem('chanPinData')).planId;
     ...
     this.axios.get(this.domain+'/commodity/getPlanList?productType=4',{headers:{"token":this.getCookie("token")}}).then(function(res) {

       if(res.data.isSuccess){
         console.log(res);
         console.log(res.data.result);
         that.reso = res.data.result;//20个意外险
        localStorage.setItem('showproduct', JSON.stringify(that.reso))
