...
mounted:{
  ...
  created(){
    ...
    localStorage.setItem('waiproduct', JSON.stringify(this.$route.query.chanpin))
    var kl = JSON.parse(localStorage.getItem('waiproduct'));
    console.log(kl);
      that.label = k.label;
  ...
  var plansId = JSON.parse(localStorage.getItem('chanPinData')).planId;
     ...
     this.axios.get(this.domain+'/commodity/getPlanList?productType=2',{headers:{"token":this.getCookie("token")}}).then(function(res) {

       if(res.data.isSuccess){
         console.log(res);
         console.log(res.data.result);
         that.reso = res.data.result;//20个意外险
        localStorage.setItem('waproduct', JSON.stringify(that.reso))
