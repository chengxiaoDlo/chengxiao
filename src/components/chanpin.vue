    ...
    <div :style='yin'>
      ...
      <div class="suggestion">
          <div class="suggestion-left">
              <div class="">
                  <h2>{{rtt.premium.substr(0,rtt.premium.indexOf('.00'))}}</h2>
                  <p>元/年</p>
              </div>
              <p>保费</p>
          </div>
...
mounted:{
  ...
  created(){
    ...
    localStorage.setItem('zhongproduct', JSON.stringify(this.$route.query.chanpin))
  ...
  var plansId = JSON.parse(localStorage.getItem('chanPinData')).planId;
     ...
     this.axios.get(this.domain+'/commodity/getPlanList?productType=1',{headers:{"token":this.getCookie("token")}}).then(function(res) {

       if(res.data.isSuccess){
         console.log(res);
         console.log(res.data.result);
         that.reso = res.data.result;//20个意外险
        localStorage.setItem('jiproduct', JSON.stringify(that.reso))
